    ...
    const renderIcon = (id) => {
        ...
        if (isInCart) {
            return (
                ...
                items-center bg-green-400 w-6 h-6 rounded-full text-black text-xl m-2'
                ><CheckIcon className="h-5 w-5 stroke-black/60" />
                </div>
            )
        } else {
                return (
                <div className='absolute top-0 right-0 flex justify-center 
                items-center bg-white w-6 h-6 rounded-full text-black text-xl m-2'
                onClick={(event) => addProductsToCart(event, data.data)}
                ><PlusIcon className="h-5 w-5 stroke-black/60" />
                </div>)
        }  
    }
    ...
    return (
        ...
        onClick={() => showProduct(data.data)}>
            <figure className='relative mb-2 w-full h-4/5'>
                <span className='absolute bottom-0 left-0 bg-white/80 rounded-lg
                text-black text-xs m-2 px-2 py-0.5'>{data.data.category.name}</span>
